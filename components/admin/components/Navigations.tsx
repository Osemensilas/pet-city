...
const Navigations = () => {
    ...
    const customerClicked = () => {
        window.history.pushState({}, '', '/admin/dashboard/customer-care');
    }

    const reportsClicked = () => {
        window.history.pushState({}, '', '/admin/dashboard/reports');
    }

    return ( 
        <>
        <div className="h-max max-h-screen min-h-screen w-[20%] bg-header hidden sm:block">
            ...
            <div className="h-max w-full flex flex-col items-between">
                <div className="h-max w-full flex flex-col gap-3">
                    ...
                    Buyers</button>
                    <button type="button" title="nav button" onClick={reportsClicked} className={`w-full py-3 text-base px-4 border-b border-grey flex items-center gap-3
                        ${pathName === "/admin/dashboard/reports" ? "text-primary" : "text-grey"}
                        `}>
                    ...
                    Shipments</button>
                    <button type="button" title="nav button" onClick={customerClicked} className={`w-full py-3 text-base px-4 border-b border-grey flex items-center gap-3
                        ${pathName === "/admin/dashboard/customer-care" ? "text-primary" : "text-grey"}
                        `}>
                    <i className="fas fa-headset"></i>
                    Customer Care</button>
                </div>
                <div className="h-max w-full px-3 mt-20">
